...
def _native_cwd_resolver(rec: dict, step: dict, cwd_state: CwdState) -> str:
    del rec
...
def _state_machine_resolver(rec: dict, step: dict, cwd_state: CwdState) -> str:
    del rec
    resolved = cwd_state.resolve_for(step["text"])
    cwd_state.observe(step["text"])
    return resolved
...
RESOLVE_CWD = {
    "claude": _native_cwd_resolver,
    "opencode": _opencode_cwd_resolver,
    "codex": _state_machine_resolver,
    "cursor": _state_machine_resolver,
}
