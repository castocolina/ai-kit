...
**Success Criteria** (what must be TRUE):
  ...
  4. The color computation is derived from a formula comparing `used_percentage` against remaining-fraction-of-window (a burn-rate signal) — e.g. 50% used with 4h remaining in a 5h window reads red, but the same 50% used with only 1h remaining reads an even more urgent color (never less urgent) — confirmed by a throwaway spike script before the formula is locked in, per the project's theory-to-hypothesis-to-spike convention. (Amended 2026-09-11: the original worked example here — "50%+ used early in a 5h window reads red, but 30% used at hour 4-of-5 can read green/blue" — used two DIFFERENT usage percentages at two different points in time, which is a burn-PACE framing. That framing is mathematically incompatible with Success Criterion 1's same-usage, less-time-left-is-more-urgent invariant: `pct/elapsed_fraction` satisfies the old worked example but is *provably monotonically decreasing* in elapsed_fraction for fixed pct — the opposite of SC1. Live-verified during Phase 8's plan-review convergence cycle 1 [opencode reviewer, confirmed arithmetically: 50%@4h-remaining→ratio 250→RED vs 50%@1h-remaining→ratio 62.5→YELLOW, backwards from SC1]; see `08-REVIEWS.md` cycle 1 and the resolved `## Plan-Revision Conflicts` entry, and `08-CONTEXT.md`'s corrected D-01–D-04. SC1 is authoritative — it is the literal, unambiguous, directly-testable "what must be TRUE" statement; the worked example here was illustrative prose that turned out to be internally inconsistent with it and is corrected to match, not the other way around.)
